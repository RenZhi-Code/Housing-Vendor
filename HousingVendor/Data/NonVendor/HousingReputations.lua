...
--
-- DATA SOURCES:
-- - Faction reputation rewards with standing requirements
--
-- CROSS-REFERENCES:
-- - HousingDecorData.lua = Master item database (name, decorID, modelFileID, iconFileID)
-- - ItemCoordinates.lua = World XYZ coordinates for items
-- - ModelPositions.lua = 3D model camera settings

if not HousingReputations then
    HousingReputations = {}
end

-- Faction rewards indexed by factionID
-- Each faction contains:
--   factionID = Numeric WoW faction ID
--   label = Faction name
--   kind = 'standard', 'friendship', or 'renown'
--   expansion = Expansion name
--   faction = 'Alliance', 'Horde', or nil (neutral)
--   group = Parent faction group (e.g., 'The Tillers')
--   vendor = NPC vendor info (npcID, name, zone, coordinates)
--   rewards = Array of {itemID, requiredStanding}
