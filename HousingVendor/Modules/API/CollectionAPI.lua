...
-- Check if item is cached as collected (session-only, no persistent cache)
local function IsItemCached(itemID)
    -- Check session cache only (cleared on reload, queried fresh from API each session)
...
-- Mark item as collected in session cache only (no persistent storage)
local function CacheItemAsCollected(itemID)
    -- Cache in session only (faster lookups, but cleared on reload)
    -- Collection status is queried fresh from API each session
...
local function GetCatalogEntryState(numericItemID)
    ...
    -- Method 1: Try GetCatalogEntryInfoByRecordID (preferred, uses decorID)
    if C_HousingCatalog.GetCatalogEntryInfoByRecordID then
        local baseInfo = nil
        local decorID = nil
    ...
    -- Method 2: Fallback to GetCatalogEntryInfoByItem (uses itemID directly)
    if not state and C_HousingCatalog.GetCatalogEntryInfoByItem then
        PrimeHousingCatalog()
...
function HousingCollectionAPI:IsItemCollected(itemID)
    ...
    -- Method 2: Check catalog entry (numStored + numPlaced)
